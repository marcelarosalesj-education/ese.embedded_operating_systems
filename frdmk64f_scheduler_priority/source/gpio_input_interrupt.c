...
struct mTaskNode* create_node(mTaskType fnt, int prio){
...
Response mTaskCreate(mTaskType newTask, int priority) {
	/* mTaskCreate function with priority */
	struct mTaskNode *node = create_node(newTask, priority);
	if(node == NULL){
...
/* Init input switch 2 and 3 GPIO. */
    ...
    if(mTaskCreate(task_cyan, 3) == R_NOT_OK){
        printf("Error creating task /n/r");
    }
    print_list();
    if(mTaskCreate(task_magenta, 5) == R_NOT_OK){
        printf("Error creating task /n/r");
    }
    print_list();
    if(mTaskCreate(task_yellow, 1) == R_NOT_OK){
        printf("Error creating task /n/r");
    }
    print_list();
    if(mTaskCreate(task_green, 2) == R_NOT_OK){
		printf("Error creating task /n/r");
	}
	print_list();
	if(mTaskCreate(task_blue, 4) == R_NOT_OK){
		printf("Error creating task /n/r");
	}
	print_list();
    /* Start Scheduler */
